/* ========================================================================== 
   QIAGEN INVENTORY - V44 (FULL OPTION - NO CODE REMOVED)
   ========================================================================== */
...
window.showForcePasswordChange = function(userKey) {
    ...
    div.innerHTML = `<div style="background:white;padding:30px;border-radius:20px;text-align:center;width:100%;max-width:320px;">
        ...
        <p style="font-size:12px;color:#666;">This is your first login. Please set a password.</p>
        <input type="password" id="new_p1" placeholder="New Password" style="width:100%;padding:12px;margin:8px 0;border:1px solid #ddd;border-radius:8px;box-sizing:border-box;">
        <input type="password" id="new_p2" placeholder="Confirm Password" style="width:100%;padding:12px;margin:8px 0;border:1px solid #ddd;border-radius:8px;box-sizing:border-box;">
        <button onclick="window.processReset('${userKey}')" style="width:100%;padding:14px;background:#f97316;color:white;border:none;border-radius:8px;font-weight:bold;cursor:pointer;">Update & Login</button>
    </div>`;
    document.body.appendChild(div);
};

window.processReset = async function(userKey) {
    const p1 = document.getElementById('new_p1').value, p2 = document.getElementById('new_p2').value;
    ...
    if (res.success) { alert("✅ Success! Logging in..."); window.location.reload(); }
};

/* ===== 2. RENDER MAIN TABLES (DEDUCT ORDER FIXED) ===== */
window.renderTable = function(data) {
    ...
    tbody.innerHTML = data.map((item, index) => {
        ...
        if (path.includes('deduct')) {
            // [Work Order] -> [Qty] -> [ปุ่มแดง]
            actionUI = `<div style="display:flex; gap:5px; align-items:center; justify-content:flex-end;">
                <input type="text" id="wo_${index}" placeholder="WO#" style="width:90px; padding:8px; border:1px solid #ddd; border-radius:6px; font-size:14px;">
            ...
            actionUI = `<div style="display:flex; gap:5px; align-items:center; justify-content:flex-end;">
                ...
                <button onclick="window.addToCart('${isWithdraw?'withdraw':'return'}','${item.Material}',${index},'${from}')" style="background:${color}; color:white; border:none; padding:10px 14px; border-radius:8px; font-weight:bold; cursor:pointer;">Add</button>
            </div>`;
        }
...
/* ===== 3. CORE INVENTORY OPERATIONS (DEDUCT, BASKET, SYNC) ===== */
window.loadStockData = async function() {
    try {
        ...
        if (res.success) { 
            ...
            if(window.renderTeamTable) window.renderTeamTable(res.data); // สำหรับหน้า Team
        }
...
window.addToCart = function(type, mat, idx, fromUser) {
    ...
    window.cart.push({ type, mat, qty: qInput.value, from: fromUser, target: (type==='return'?'0243':user) });
...
window.showReviewModal = function() {
    let html = `<div style="max-height:250px; overflow-y:auto; margin:15px 0;"><table style="width:100%; font-size:11px; border-collapse:collapse;">
        <tr style="background:#eee;"><th style="padding:8px;text-align:left;">Mat</th><th style="padding:8px;">Qty</th><th style="padding:8px;">Transfer</th><th style="padding:8px;"></th></tr>`;
    window.cart.forEach((i, idx) => {
        html += `<tr style="border-bottom:1px solid #eee;">
            <td style="padding:8px;">${i.mat}</td>
            <td style="padding:8px;">${i.qty}</td>
            <td style="padding:8px;">${i.from} → ${i.target}</td>
            <td style="padding:8px;"><button onclick="window.removeFromCart(${idx})" style="color:red; border:none; background:none; font-weight:bold; font-size:16px;">✕</button></td>
        </tr>`;
    });
    html += `</table></div>`;
    ...
    div.innerHTML = `<div style="background:white; width:100%; max-width:400px; border-radius:20px; padding:25px; box-sizing:border-box;">
        <h3 style="margin:0; color:#003366;">Confirm Transactions</h3>${html}
        <button id="sync-btn" onclick="window.confirmSendAndSync()" style="width:100%; padding:14px; background:#003366; color:white; border:none; border-radius:12px; font-weight:bold; margin-top:10px;">Sync & Send Email</button>
        <button onclick="document.getElementById('review-modal').remove()" style="width:100%; margin-top:10px; background:none; border:none; color:#666; font-weight:bold;">Cancel / Close</button>
    </div>`;
    document.body.appendChild(div);
};
...
window.confirmSendAndSync = async function() {
    ...
    try {
        for (const item of window.cart) {
            // เรียกใช้ Action ตามประเภท (withdraw/return/transfer)
    ...
    } catch (e) { alert("❌ Sync Failed: " + e.message); btn.disabled = false; }
};
