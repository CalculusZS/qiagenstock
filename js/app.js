/* ========================================================================== 
   QIAGEN INVENTORY - FULL VERSION (NO CUT) - FIXED LOGIC BASED ON BACKEND V9
   ========================================================================== */
...
window.showForcePasswordChange = function(userKey) {
    ...
    div.innerHTML = `<div style="background:white;padding:30px;border-radius:20px;text-align:center;width:320px;">
        <h3 style="color:#f97316;margin-bottom:15px;">Set New Password</h3>
        ...
        <button onclick="window.processReset('${userKey}')" style="width:100%;padding:14px;background:#f97316;color:white;border:none;border-radius:8px;font-weight:bold;margin-top:10px;">Update & Login</button>
    </div>`;
...
/* 2. RENDER TABLE & SEARCH (แก้ไขข้อ 5) */
window.renderTable = function(data) {
    ...
    tbody.innerHTML = data.map((item, index) => {
        ...
        if (path.includes('deduct')) {
            // แก้ไขข้อ 4: Deduct (From = User, To = "-", WO = Input)
            actionUI = `<div style="display:flex; gap:5px; justify-content:flex-end; align-items:center;">
                ...
                <input type="number" id="qty_${index}" value="1" style="width:40px; text-align:center; padding:8px; border:1px solid #ddd;">
            ...
            // แก้ไขข้อ 2 & 3: From/To ตาม Log ที่ต้องการ
            ...
            actionUI = `<div style="display:flex; gap:5px; justify-content:flex-end; align-items:center;">
                ...
                <button onclick="window.addToCart('${isW?'withdraw':'return'}','${item.Material}',${index},'${from}','${target}')" style="background:${color}; color:white; border:none; padding:8px 14px; border-radius:5px; font-weight:bold;">ADD</button>
            </div>`;
        }
...
window.doDeduct = async function(mat, idx) {
    ...
    // ข้อ 4: Log (From=User, To="-", WO=wo) ตาม handleDeductWithWO(p)
    const url = `${API}?action=deduct&user=${encodeURIComponent(user)}&material=${encodeURIComponent(mat)}&qty=${qty}&wo=${encodeURIComponent(wo)}&pass=${MASTER_PASS}`;
    try {
        const res = await fetch(url).then(r => r.json());
        if (res.success) { alert("✅ Deducted & Logged"); window.loadStockData(); }
...
window.confirmSendAndSync = async function() {
    ...
    try {
        for (const item of window.cart) {
            // ข้อ 1, 2, 3: เรียก handleSync(p, type) ใน GAS
            // GAS ใช้ p.from และ p.user (user ในที่นี้คือ To)
    ...
    } catch (e) { alert("❌ Sync Error"); btn.disabled = false; btn.innerText = "Sync & Open Outlook"; }
};
...
window.showReviewModal = function() {
    let html = `<div style="max-height:300px; overflow-y:auto; margin:15px 0; font-size:12px;">
        <table style="width:100%; border-collapse:collapse;">
            <tr style="background:#f1f5f9;"><th style="padding:8px; text-align:left;">Material</th><th style="padding:8px;">Qty</th><th style="padding:8px;">From/To</th></tr>`;
    window.cart.forEach((i, idx) => {
        html += `<tr style="border-bottom:1px solid #eee;">
            <td style="padding:8px;">${i.mat}</td>
            <td align="center">${i.qty}</td>
            <td align="center">${i.from}→${i.target}</td>
            <td><button onclick="window.removeFromCart(${idx})" style="color:red; border:none; background:none;">✕</button></td>
        </tr>`;
    });
    html += `</table></div>`;
    ...
    div.innerHTML = `<div style="background:white; width:90%; max-width:400px; border-radius:20px; padding:20px;">
        <h3>Review Transactions</h3>${html}
        <button id="sync-btn" onclick="window.confirmSendAndSync()" style="width:100%; padding:15px; background:#0ea5e9; color:white; border:none; border-radius:12px; font-weight:bold; margin-top:10px;">Sync & Open Outlook</button>
