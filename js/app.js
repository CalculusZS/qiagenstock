...
window.addToCart = function(type, mat, idx, from, target) {
    ...
    const qVal = document.getElementById(qID).value;
...
// ระบบ Search ที่รองรับทั้งหน้า Team และหน้าหลัก
...
window.renderTeamTable = function(data) {
    ...
    container.innerHTML = html || '<tr><td colspan="2" align="center" style="padding:20px;">No matching results.</td></tr>';
};
...
window.renderTable = function(data) {
    ...
    let rowsHtml = data.map((item, index) => {
        ...
        if (path.includes('showall')) {
            ...
            const btnColor = isW ? '#003366' : '#16a34a';
            const btnName = isW ? 'Withdraw' : 'Return';
            actionUI = `<div style="display:flex; gap:5px; justify-content:flex-end; align-items:center;">
                        <input type="number" id="qty_${index}" value="1" style="width:40px; padding:6px; text-align:center; border:1px solid #ddd; border-radius:6px;">
                        <button onclick="window.addToCart('${isW?'withdraw':'return'}','${item.Material}',${index},'${isW?'0243':user}','${isW?user:'0243'}')" style="background:${btnColor}; color:white; border:none; padding:8px 12px; border-radius:6px; font-weight:bold;">${btnName}</button></div>`;
        }
...
// ปรับปรุงให้ดึงข้อมูลไวขึ้นด้วยระบบ Cache
window.loadStockData = async function() {
    const cacheKey = 'qiagen_stock_cache';
    const cacheTimeKey = 'qiagen_stock_time';
    const now = new Date().getTime();
    
    // ดึงข้อมูลจาก Cache ก่อนเพื่อความไว (ถ้าอายุไม่เกิน 30 วินาที)
    const cachedData = localStorage.getItem(cacheKey);
    const cachedTime = localStorage.getItem(cacheTimeKey);
    
    if (cachedData && cachedTime && (now - cachedTime < 30000)) {
        window.allRows = JSON.parse(cachedData);
...
window.doDeduct = async function(mat, idx) {
    ...
    try {
        ...
        if (res.success) { 
            ...
            localStorage.removeItem('qiagen_stock_time'); // ล้าง Cache เพื่อบังคับโหลดใหม่
            window.loadStockData(); 
        }
    } catch(e) { alert("❌ Error"); }
};

window.logout = () => { sessionStorage.clear(); localStorage.removeItem('qiagen_cart'); localStorage.removeItem('qiagen_stock_cache'); window.location.replace('index.html'); };
