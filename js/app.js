    ...
    try {
        ...
        if (res.success) {
            listDiv.innerHTML = res.data.map(row => {
                ...
                return `
                <div style="display:flex; border-bottom:1px solid #eee; padding:12px; font-size:12px; min-width:1100px; align-items:center;">
                    <div style="flex:1;">${new Date(row[0]).toLocaleString()}</div>
                    <div style="flex:1.2; font-weight:bold; color:#003366;">${row[1]}</div>
                    <div style="flex:1.5;">${row[4] || '-'}</div>
                    <div style="flex:1; text-align:center;">
                        <span style="padding:4px 8px; border-radius:4px; font-weight:bold; background:${type === 'withdraw' ? '#e1effe' : (type === 'return' ? '#def7ec' : '#fde8e8')}; color:${type === 'withdraw' ? '#1e42af' : (type === 'return' ? '#03543f' : '#9b1c1c')};">
                            ${type.toUpperCase()}
                        </span>
                    </div>
                    <div style="flex:0.5; text-align:center; font-weight:bold;">${row[6]}</div>
                    <div style="flex:1; text-align:center; font-weight:bold;">${row[7] || '-'}</div>
                    <div style="flex:1; text-align:right; color:#ef4444; font-weight:bold;">${row[8] || '-'}</div>
                </div>`;
            }).reverse().join('');
        }
