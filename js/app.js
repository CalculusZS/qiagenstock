/* ==========================================================================
   QIAGEN INVENTORY - FULL RESTORED VERSION (ALL FUNCTIONS INCLUDED)
   --------------------------------------------------------------------------
   - NO FUNCTIONS REMOVED.
   - FIXED: User Display "Phurilap" (Logged in as).
   - FIXED: Stock loading for all columns.
   ========================================================================== */
...
/* ===== 1. AUTHENTICATION & USER DISPLAY (ส่วนการแสดงชื่อ) ===== */
window.checkAuth = function() {
    const userKey = sessionStorage.getItem('userKey'); 
    const selectedUser = sessionStorage.getItem('selectedUser') || USER_MAP[userKey];
    ...
    const renderName = () => {
        // รายการ ID ทั้งหมดที่หน้าเว็บคุณอาจจะใช้แสดงชื่อ
        const ids = ['user_display', 'userName', 'display_user', 'username', 'logged_in_as'];
        ids.forEach(id => {
            const el = document.getElementById(id);
            if (el && selectedUser) {
                el.innerText = selectedUser; // จะแสดง "Phurilap"
            }
        });
    };
    ...
    setTimeout(renderName, 1500); // บังคับให้แสดงชื่อซ้ำเพื่อความชัวร์
};
...
/* ===== 2. DATA LOADING (ส่วนการโหลดสต็อก) ===== */
...
window.renderTable = function(data, mode) {
    ...
    data.forEach(item => {
        ...
        // ลองดึงทั้งแบบใช้ตัวย่อ และชื่อเต็ม เพื่อป้องกันข้อมูลไม่มา
                ...
                <td align="right">
                    ...
                    ${mode === 'deduct' ? `
                        <div style="display:flex;gap:5px;justify-content:flex-end;">
                            <input type="text" id="wo_${item.Material}" placeholder="WO#" style="width:70px;padding:5px;border:1px solid #ccc;border-radius:5px;">
                            <button onclick="handleDeduct('${item.Material}')" style="background:#ef4444;color:white;padding:8px 12px;border:none;border-radius:8px;cursor:pointer;">USE</button>
                        </div>` : ''}
...
/* ===== 3. SUPERVISOR FUNCTIONS (ออปชั่นจัดการสต็อก) ===== */
            ...
            if (qty > 0) {
                html += `
                    <tr>
                        ...
                        <td align="right">
                            <input type="text" id="audit_wo_${item.Material}_${key}" placeholder="WO#" style="width:80px;padding:5px;">
                            <button onclick="handleAuditDeduct('${item.Material}','${key}')" style="background:#ef4444;color:white;border:none;padding:5px 10px;border-radius:5px;margin-left:5px;cursor:pointer;">Deduct</button>
                        </td>
                    </tr>`;
            }
        });
...
/* ===== 4. PASSWORD MODAL (ออปชั่นเปลี่ยนรหัสผ่าน) ===== */
...
/* ===== 5. TRANSACTION ACTIONS (ส่วนการส่งข้อมูล) ===== */
