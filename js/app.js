...
/* --- 1. LOGIN & FORCE PASSWORD CHANGE (กู้คืนหน้าเปลี่ยนรหัส) --- */
window.handleLogin = async function() {
    ...
    try {
        ...
        if (res && res.success) {
            ...
            // ถ้า Status คือ NEW ให้บังคับเปลี่ยนรหัสทันที
            if (res.status === 'NEW') { 
...
window.showForcePasswordChange = function(userKey) {
    ...
    div.style = "position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.95);display:flex;justify-content:center;align-items:center;z-index:20000;padding:20px;";
    div.innerHTML = `
        <div style="background:white;padding:30px;border-radius:20px;text-align:center;width:100%;max-width:320px;">
            <h2 style="color:#f97316;">Set New Password</h2>
            <p style="font-size:13px;color:#666;margin-bottom:15px;">Please set your password before first login.</p>
...
/* --- 2. LOGOUT FUNCTION (กู้คืนปุ่ม LOGOUT) --- */
...
/* --- 3. SEARCH SYSTEM (แก้ไขการค้นหาให้ใช้งานได้จริง) --- */
...
window.confirmSendAndSync = async function() {
    ...
    try {
        ...
        window.location.replace(`mailto:${to}?subject=${subject}&body=${body}`);
...
window.addToCart = function(t, m, i, f, tg) {
    const q = document.getElementById('qty_' + i).value;
    const itm = window.allRows.find(x => String(x.Material) === String(m));
    window.cart.push({ type:t, mat:m, name:itm['Product Name'], qty:q, from:f, target:tg });
...
window.updateCartUI = function() {
    ...
    btn.innerHTML = window.cart.length > 0 ? `<button onclick="window.showReviewModal()" style="background:#0ea5e9;color:white;padding:12px 20px;border-radius:50px;border:none;font-weight:bold;">Cart (${window.cart.length})</button>` : '';
};
...
window.showReviewModal = function() {
    ...
    div.innerHTML = `<div style="background:white;width:85%;border-radius:15px;padding:20px;"><h3>Review Sync</h3>${html}<button id="sync-btn" onclick="window.confirmSendAndSync()" style="width:100%;padding:12px;background:#0ea5e9;color:white;border-radius:8px;border:none;margin-top:10px;">Confirm & Sync</button><button onclick="this.parentElement.parentElement.remove()" style="width:100%;margin-top:10px;color:gray;border:none;background:none;">Close</button></div>`;
    document.body.appendChild(div);
};
