/* ========================================================================== 
   QIAGEN INVENTORY - V32 (ULTIMATE STABLE)
   ========================================================================== */
...
window.handleLogin = async function() {
    ...
    try {
        ...
        if (res && res.success) {
            ...
            // เช็คสถานะ NEW เพื่อบังคับเปลี่ยนรหัสผ่าน
            if (res.status === 'NEW') {
...
window.showForcePasswordChange = function(userKey) {
    // ล้าง overlay เก่าถ้ามี
    const old = document.getElementById('force-pw-overlay');
    if (old) old.remove();
...
window.renderTable = function(data) {
    ...
    tbody.innerHTML = data.map((item, index) => {
        ...
        if (path.includes('withdraw')) {
            ...
            // ปุ่ม DEDUCT สีแดง (#ef4444) พร้อมช่อง Qty และ WO
            actionUI = `
                <div style="display:flex; gap:5px; align-items:center; justify-content:flex-end;">
                    ...
                    <button onclick="window.doDeduct('${item.Material}', ${index})" 
                        style="background:#ef4444; color:white; border:none; padding:10px 12px; border-radius:8px; font-weight:bold; cursor:pointer;">
                        Deduct
                    </button>
                </div>`;
        }
...
/* ===== (ฟังก์ชันอื่นๆ: Basket, Sync, Logout - คงเดิมเพื่อให้ระบบไม่พัง) ===== */
...
window.showReviewModal = function() {
    ...
    div.innerHTML = `<div style="background:white; width:100%; max-width:400px; border-radius:15px; padding:20px; box-sizing:border-box;">
        ...
        <button id="sync-btn" onclick="window.confirmSendAndSync()" style="width:100%; padding:14px; background:#003366; color:white; border:none; border-radius:10px; font-weight:bold; cursor:pointer;">Sync & Send Email</button>
        <button onclick="document.getElementById('review-modal').remove()" style="width:100%; margin-top:10px; background:none; border:none; color:#666; cursor:pointer; width:100%;">Close</button>
    </div>`;
    document.body.appendChild(div);
};
